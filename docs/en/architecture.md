...
TUI settings group Appearance, Models & Connection, Agent, Web, Development, Permissions & Security, and Advanced. Wide terminals show category navigation; terminals narrower than 88 columns use a single scrolling column. Search matches names, effective values, and descriptions across all groups; Escape clears search before closing. The overview shows managed global defaults and safe provider status, while reasoning shows the requested level rather than inferred model capability. Detail screens retain existing controllers and persistence ports. Connection settings retain their reload flow; no permission bypass or nonfunctional roadmap control is introduced.

## Persistent agent settings

`/settings` persists validated `AgentPreferences` in the `agent` object of `ui-preferences.json`; additive optional fields remain readable by the existing version-1 JSON format. Other preference writes preserve it. Each page can restore inheritance. Saving does not execute commands or rerun the current task. Runtime-affecting values use the controlled composition reload and resume the same session; interface-only values apply immediately.
...
Settings groups Appearance, Models & Connection, Agent, Web, Development, Permissions & Security, and Advanced. Normal forms show effective values and one of four sources: Default, User, Project, or CLI. User scope means all projects. Project-scoped preferences remain keyed by normalized workspace path in the user state file; they are separate from SessionProject identity and never write to the repository. Explicit CLI values are labeled.

The normal Web form offers Search Off, Auto, and Custom, and Fetch Off or Auto. Auto uses the existing capability-aware resolver. Custom lists only configured profiles with available credentials and a trusted executable Search backend; it does not infer compatibility from provider names. When no candidate exists, the form shows the runtime blocker and links to Manage Providers when available. Advanced retains raw route modes and all earlier execution, request, LSP, verification, context, alert, and wake controls. Capability status shows the resolved path, safe profile/model labels, and typed reason without endpoint or credential data.

Runtime-affecting saves reuse the controlled TUI reload seam. Bootstrap reloads configuration and preferences, then passes the same active Session ID through the existing resume path. A running turn blocks a runtime save before persistence. Interface-only preferences apply immediately. Existing provider, permission, sandbox, tool-contract, and routing boundaries remain authoritative. See [ADR 0175](adr/0175-effective-settings-and-web-capability-ux-v2.md).
