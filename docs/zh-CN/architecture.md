...
TUI 设置按外观、模型与连接、Agent、网页能力、开发、权限与安全、高级分组。宽终端提供分类侧栏；小于 88 列时采用单列滚动布局。搜索匹配全部分类中的名称、有效值和说明，Esc 先清除搜索再关闭。首页展示受管默认值和安全的供应商状态；推理强度展示请求值而非猜测模型能力。子页面沿用现有控制器和存储端口。连接设置仍使用既有重新加载流程，不增加权限绕过或无效规划开关。

## 可保存的 Agent 设置

`/settings` 通过 `UiPreferencesStore` 将经过校验的 `AgentPreferences` 保存至 `ui-preferences.json` 的 `agent` 对象；新增可选字段与现有 version-1 JSON 格式兼容，其他偏好写入会保留该对象。每页可恢复继承。保存不会执行命令或重跑当前轮次；影响运行时的设置会受控重建 composition 并恢复同一 Session，纯界面设置立即生效。
...
Agent 偏好还包括 Enter 行为、输入折行、轮次结束与失败铃声、每会话唤醒上限、唤醒冷却间隔、压缩保留条目数、摘要 token 上限及选中的 Search profile。新增项同样经过类型与范围校验。压缩参数注入现有 `ContextCompactionPlanner`，不改变触发边界、工具配对保护或上下文容量；唤醒参数使用现有 `BackgroundWakeLimits`，不会开启原本关闭的唤醒策略。铃声默认关闭，受终端配置控制，主动取消不发失败提醒。Enter 换行模式使用发送按钮提交，Ctrl+J/F2 保持换行功能。

所有偏好项支持用户与当前工作区两个保存范围。工作区覆盖保存在用户 `ui-preferences.json` 的 `projects` 对象中，以规范化绝对工作区路径的 SHA-256 为键；不读取或写入仓库配置，不自动接受检出中的执行指令。非空工作区值覆盖用户值，空值继承；基础配置及显式 CLI 参数的既有优先级保持不变。保存界面先重读对应范围，只替换本页字段；其他页面、范围与未知字段由原子存储保留。切换范围会丢弃本页未保存编辑。高级查看页显示启动值、已保存值和来源，区分 CLI 生效值与用户偏好；验证命令只显示是否设置。

设置导航的宽屏初始视图聚焦“外观”，仍可主动选择全部设置或跨分类搜索。分类使用柔和底色，条目之间用细线与留白分隔；详细表单加强字段标题和说明间距。窄屏继续以单列展示全部分类。

## 设置体验 V2

设置分为外观、模型与连接、Agent、网页能力、开发、权限与安全、高级。普通表单显示有效值及其来源：默认、用户、项目或 CLI。用户范围对应所有项目。项目偏好仍在用户 state 文件中按规范化工作区路径区分；它与 SessionProject identity 独立，不写入仓库。显式 CLI 值会明确标注。

普通网页表单提供 Search 关闭、自动、自定义，以及 Fetch 关闭或自动。自动模式使用现有能力解析器。自定义模式只列出凭据可用且可信 Search 后端可执行的已配置 profile，不根据供应商名称猜测兼容性。没有候选项时显示运行时阻塞原因，并在可用时链接到供应商管理。高级页面保留原始路由模式及此前的执行、请求、LSP、验证、上下文、提醒和唤醒控制。能力状态显示已解析路径、安全的 profile/model 标签和类型化原因，不包含端点或凭据。

影响运行时的保存复用受控 TUI reload seam。Bootstrap 重新读取配置和偏好，再经既有 resume 路径传递同一活动 Session ID。运行中的轮次会在持久化前阻止运行时设置保存；纯界面偏好立即生效。供应商、权限、沙箱、工具契约和路由边界仍由现有组件负责。详见 [ADR 0175](adr/0175-effective-settings-and-web-capability-ux-v2.md)。
