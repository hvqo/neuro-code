...
**请求上下文只包含按 generation 固定的 index，不包含全部正文。** 项目绑定的 Main Agent 会在仓库指令和 Skills 之后、会话历史之前读取有界 index。即使后台提取更新持久化 store，该应用层快照在当前 active context generation 中仍保持字节稳定。新建或恢复 Session、项目 attach/move/detach，以及已提交的 Fresh Context Rollover 会刷新快照；重命名保留相同的 `SessionProject.id` 和快照。Detach 或删除后会立即清除 active scope。该消息有专用的 synthetic reason，不会成为持久会话历史。Index 和 `read_project_memory` 工具都将记忆标注为上下文证据，而非指令；当前仓库、Git 状态和 `AGENTS.md` 始终优先。工具定义无论当前是否绑定项目都保持稳定；无项目时执行会 fail closed。该工具只接受一个精确记忆身份，并通过受当前 binding scope 限制的应用只读服务读取，不能读取 state root 中的任意路径。Subagent binding 默认不获得此 scope。

**上下文形态面向缓存稳定性。** 请求按 `Stable Prefix → Append-only Conversation → Volatile Tail` 排列。Project Instructions、Skills 和固定的 Project Memory index 位于持久对话之前；高频 Working Set 与 runtime notices 位于对话之后。后台提取不会追溯改写 active prefix。Full compaction 不刷新 Memory snapshot，因为它不是 context-generation boundary；已提交的 Fresh Context Rollover 会刷新。Project Memory、Working Set 以及未来的 microcompaction/compaction 都必须同时权衡 token reduction、cache preservation 和 correctness。历史上下文只在明确的缓存失效边界，或有测量证明收益足够时才追溯改写。本决策不增加 Provider 专属 cache key、breakpoint、prewarm 或 Runtime trace。
