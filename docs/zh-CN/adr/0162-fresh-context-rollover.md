...
也不能把 transient runtime context 当作 conversation history。持久 marker 还必须让 control 与下一次
model request 之间发生 crash 时保持确定性。

## 决策

在现有 `sessions` row 增加一个单调递增的 `context_generation` integer。Schema version 32 以非负
default 增加该 column，并以 additive 方式迁移 legacy schema-31 database。`SessionStore` 负责读取和
原子递增 marker；`SessionContextRolloverApplicationService` 是 application boundary。该 marker 是
session metadata，不是复制的 history sequence、summary 或 Working Set row。
...
system messages 和本回合新的 user input 开始；旧 durable item 仍在完整 sequence 中，但不会注入这个
active projection。成功执行唯一的 `new_context` call 后，也会立即安装同样的 projection boundary：system
prefix 和 current user message 作为下一次 request 的 seed，随后是新的 generation notice 以及之后的
runtime/model/tool item。Instructions、skills 和 current Working Set 由既有 `ContextBuilder`/Working Set
路径为该 request 重新构建。
...
tool/event/finalization path；session marker 在下一次 model request 前已经提交，所以即使 process 在
turn finalization 前停止，marker 仍是 recovery 的 source of truth。后续 run 会在 model 执行前根据持久
generation 安装相同的 fresh projection。既有 unresolved-turn recovery 规则仍然适用；rollover 不会
自动 replay Provider request。
...
data 仍可读取。Fork session 保留现有 history-copy 行为，但使用 default generation，也不会继承 CM2
Working Set state。聚焦测试覆盖回合内 fresh projection、Working Set 与 canonical history 保留、写入前
output-limit 拒绝以及下一次 model step 被中断后的 generation recovery。Ruff、format、聚焦 mypy 和文档
parity 是本变更的 change-level check；仓库完整 CI matrix 仍是最终的完整验证 gate。
